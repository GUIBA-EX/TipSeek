...
CPU parallelism defaults to `-p auto`: TStools counts physical cores allowed by affinity/cpuset, then applies cgroup or scheduler limits; an explicit integer overrides detection. The scheduler keeps the original 1--2-unit recruitment reservation separate from UCEFilter's supported single compute worker.
