...
The gene assembler extends the highest-weight edge first and keeps alternatives for bounded backtracking. It retains up to three candidates per side before combining and scoring them. `gene` builds family summaries from those candidates; `exon` derives proteins from coding nucleotide baits by default, then adds miniprot annotation and structural validation. External `.faa` references are optional. Use `--reuse-reference-cache` to reuse the validated reference k-mer cache. Candidate resolution is documented in the [gene workflow guide](gene_EN.md); exon-model QC and N-padding are documented in the [exon annotation guide](exon_EN.md).
