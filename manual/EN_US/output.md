...
**uce_contigs**: phyluce-compatible contig export generated by UCE assembly mode. It contains one `*.contigs.fasta` file per sample. `sample_name_map.tsv` records the mapping between TipSeek sample names (including retained legacy GeneMiner2-compatible identifiers) and phyluce-safe sample names.
